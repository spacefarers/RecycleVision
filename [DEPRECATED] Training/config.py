...
class QATConfig:
    ...
    enabled: bool = False
    backend: str = "qnnpack"  # "qnnpack" or "fbgemm"
    per_channel: bool = False  # Use per-channel quantization
    calibration_batches: int = 32
    calibration_strategy: str = "random"  # "random", "stratified", "uniform"
    log_quantization_every_n_epochs: int = 5
    verbose: bool = True

    def __post_init__(self) -> None:
        if self.backend not in ["qnnpack", "fbgemm", "x86"]:
            self.backend = "qnnpack"
        if self.calibration_strategy not in ["random", "stratified", "uniform"]:
            self.calibration_strategy = "random"
