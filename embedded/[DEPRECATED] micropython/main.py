...
# ====== IMPORTANT: Quantization Parameters ======
# After recompiling quantize.py with int8 input type, extract these from nncase logs:
# Run: python quantize.py --calibration-dir data/sorted_2_class 2>&1 | grep -i "scale\|zero"
# Then replace these values:
INPUT_SCALE = 0.02135      # TODO: Replace with actual scale from nncase (e.g., 0.02135)
INPUT_ZERO_POINT = -2      # TODO: Replace with actual zero_point from nncase (e.g., -2)

# ImageNet normalization (must match quantize.py)
...
def quantize_to_int8(float_tensor):
    """
    Convert normalized float32 tensor to int8 for KPU input.

    Formula: int8 = round(float / scale) + zero_point

    Args:
        float_tensor: Shape (1, 3, H, W), normalized with ImageNet mean/std

    Returns:
        int8 tensor clipped to [-128, 127]
    """
    # Quantize: int8 = round(float / scale) + zero_point
    int8_data = np.round(float_tensor / INPUT_SCALE) + INPUT_ZERO_POINT
    # Clip to valid int8 range
    int8_data = np.clip(int8_data, -128, 127).astype(np.int8)
    return int8_data
...
try:
    while True:
        ...
        # Preprocess: Convert to float32, normalize, and quantize
        float_rgb = rgb_hwc.astype(np.float32) / 255.0  # [0, 1]
        float_nchw = np.transpose(float_rgb, (2,0,1)).reshape((1, 3, H, W))  # NCHW

        # ImageNet normalization (must match quantize.py)
        normalized = (float_nchw - IMAGENET_MEAN) / IMAGENET_STD

        # Quantize to int8 for KPU
        int8_nchw = quantize_to_int8(normalized)

        if ai2d_builder is None:
            ai2d_builder = ai2d.build([1,3,H,W], MODEL_IN_NCHW)

        ai2d_input_tensor = nn.from_numpy(int8_nchw)
        ...
        del ai2d_input_tensor, float_rgb, float_nchw, normalized, int8_nchw, rgb_hwc, img, out_tensor
        gc.collect()
        dt = time.ticks_ms() - t0
        if dt > 0:
            print("FPS:", 1000.0 / dt)
